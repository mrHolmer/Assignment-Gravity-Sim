...
async fn main() {  // This is the function that is normally set to immediately execute on starting the program. 
	...
	planetary_bodies.push(PlanetaryBody {mass: 1.0, radius: (macroquad::prelude::screen_height() as f64) / 20.0, velocity: [0.0, 5.0], location: [{0.0 - {macroquad::prelude::screen_width() * 0.25}} as f64, 0.0]});
